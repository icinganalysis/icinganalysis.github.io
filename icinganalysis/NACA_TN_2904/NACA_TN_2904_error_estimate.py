...
alt_ft = 10000
tk = (9 + 459.67) / 1.8
p = langmuir_cylinder_values.calc_pressure(alt_ft * 12 * 0.0254)
...
for mph in mphs:
    ...
    for mvd in mvds:
        ...
        (
            ...
            dist_ramped_up,
            rss_ramped_up,
        ) = mc.find_lwc_mvd_dist(tk, u, p, masses_ramped_up, guess_mvd=mvd)
        fds_up.append((abs(mvd_ramped_up - mvd) / mvd))
        dists_up.append(dist_ramped_up)
        masses_ramped_down = [
            mass * ratio for mass, ratio in zip(masses, reversed(mass_ramp_up_ratios))
        ]
        (
            lwc_ramped_down,
            mvd_ramped_down,
            dist_ramped_down,
            rss_ramped_down,
        ) = mc.find_lwc_mvd_dist(tk, u, p, masses_ramped_down, guess_mvd=mvd)
