...
from icinganalysis.langmuir_cylinder_values import calc_k, valid_distribution_ids
...
def calc_mass_average_em_diameter_with_distribution(
    ...
    for i in range(10):
        ...
        em_initial = calc_em_from(
            calc_k(tk, u, mvd, initial_diameter), k_phi, distribution
        )
        mass_initial = (
            em_initial * lwc / 1000 * u * initial_diameter * length * time_in_icing
        )
        em_final = calc_em_from(
            calc_k(tk, u, mvd, initial_diameter), k_phi, distribution
        )
...
class Multicylinder:
    ...
    def calc_ems_fixed_diameters(self, tk, ms, p, mvd, distribution="Langmuir A"):
        ...
        for d, length in zip(self.diameters, self.lengths):
            em = self.calc_em_with_distribution_to_use(
                calc_k(tk, ms, mvd, d), calc_k_phi(tk, p, ms, mvd), distribution
            )
            ems.append(em)
        return ems
    ...
    def find_lwc_mvd_from_dist(
        ...
        mvd_lwc_solution = minimize(
            f_log10_rss,
            x0=(guess_mvd, guess_lwc),
            bounds=[(1, 1000), (0.01, 10)],  # assumed x0
        )
...
if __name__ == "__main__":
    ...
    inv_kcs = [1 / calc_k(tk, u, mvd, d) for d in ds]
