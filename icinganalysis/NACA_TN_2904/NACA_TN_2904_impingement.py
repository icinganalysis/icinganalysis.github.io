...
from icinganalysis.langmuir_cylinder import get_mids, langmuir_lwc_fractions, valid_distribution_ids
...
_k_interps = {
    'Langmuir A': (
        ...
        interp1d(log10_ks, data_table_iv_augmented[200000]['ems_a'], kind, fill_value='extrapolate'),
    ),
    'Langmuir B': (
        ...
        interp1d(log10_ks, data_table_iv_augmented[200000]['ems_b'], kind, fill_value='extrapolate'),
    ),
    'Langmuir C': (
        ...
        interp1d(log10_ks, data_table_iv_augmented[200000]['ems_c'], kind, fill_value='extrapolate'),
    ),
    'Langmuir D': (
        ...
        interp1d(log10_ks, data_table_iv_augmented[200000]['ems_d'], kind, fill_value='extrapolate'),
    ),
    'Langmuir E': (
        ...
        interp1d(log10_ks, data_table_iv_augmented[200000]['ems_e'], kind, fill_value='extrapolate'),
    ),
}

lkphis = [log10(_) for _ in (10**l10phi_for_k_phi_0, 200, 1000, 3000, 10000, 50000, 200000)]


def ie(k, k_phi, distribution='Langmuir A'):
    ems = [ki(log10(1 / k)) for ki in _k_interps[distribution]]

    _ip = interp1d(lkphis, ems,
                   kind='linear',
                   # kind='quadratic',
...
def make_table_iii():
    ...
    for i, inv_k in enumerate(data_table_iv_augmented[k_phi]['inv_ks']):
        ...
        for dr, w in zip(get_mids(distribution), langmuir_lwc_fractions):
            ...
            # emz +=
        print(k_phi, phi, inv_k, distribution, ems, emy, data_table_iv_augmented[k_phi]['ems_c'][i])
    k_phi = 50000
...
if __name__ == '__main__':
    ...
    for phi in data_fig_6:
        ...
        ks = plt.np.logspace(log10(.125), 4)
        ems = [calc_em_naca_tn_2904_from(k, phi) for k in ks]
