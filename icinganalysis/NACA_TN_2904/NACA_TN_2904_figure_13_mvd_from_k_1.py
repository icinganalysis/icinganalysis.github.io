...
from icinganalysis.NACA_TN_2904 import (
    NACA_TN_2904_impingement,
    multicylinder_naca_tn_2904,
)


def c1(d_inch, mph, mvd, tk):
    mu_slug_ft_s = langmuir_cylinder_values.calc_air_viscosity(tk) / 14.5939 * 0.3048
...
def calc_mvd_from_inv_k_for_L_1_inch(inv_k_for_L_1_inch, tk, u):
    mvd = langmuir_cylinder_values.calc_d_drop_from_k(
        1 / inv_k_for_L_1_inch, tk, u, 2 * 1 * 0.0254,  # D = 2 * L * m/inch
    )
    return mvd


def calc_lwc_masses_to_match_total_mass(
    masses,
    d_cyls,
    length_cyls,
    tk,
    ...
    masses_calc = [
        1 / 1000 * em * u * time_in_icing * d * l
        for d, l, em in zip(d_cyls, length_cyls, ems)
    ]
    lwc = sum(masses) / sum(masses_calc)
    masses_calc = [
        lwc / 1000 * em * u * time_in_icing * d * l
        for d, l, em in zip(d_cyls, length_cyls, ems)
    ]
...
def get_data(
    inv_k_for_L_1_inch, masses, d_cyls, length_cyls, tk, p, u, distribution="Langmuir A"
):
    mvd = calc_mvd_from_inv_k_for_L_1_inch(inv_k_for_L_1_inch, tk, u)
    lwc, masses_calc = calc_lwc_masses_to_match_total_mass(
        masses, d_cyls, length_cyls, tk, p, u, mvd, distribution
    )
...
if __name__ == "__main__":
    ...
    data = (
        (
            "Langmuir A",
            0.165,
            [mass * ratio for mass, ratio in zip(masses, mass_ramp_up_ratios)],
        ),
        ...
        (
            ...
            [
                mass * ratio
                for mass, ratio in zip(masses, reversed(mass_ramp_up_ratios))
            ],
        ),
    )

    for distribution, inv_k_for_L_1_inch_nominal, masses in data:
        lwc, mvd, rss = get_data(
            inv_k_for_L_1_inch_nominal,
            masses,
            d_cyls,
            lengths_cyls,
            tk,
