...
if __name__ == "__main__":
    ...
    reference_calculated_masses, reference_average_ems, reference_average_dias = mc.calc_masses_average_ems_diameters(tk, p, u, lwc, mvd, time, assumed_distribution, ice_density)
    ...
    appearent_mass_2 = 0.03/1000*100/1.
    print(apparent_mass_1, appearent_mass_2)
    print(0.02/1000, 0.03/1000)
    lwc_percent_errors = []
    mvd_percent_errors = []
    header = ['Source of error', 'Estimated amount of error', 'Resultant percent error (E) Water content', 'Resultant percent error (E) Drop diameter']
    rows = []
    ...
    lwc_percents = []
    mvd_percents = []
    print(reference_calculated_masses[0], delta_weight_grams/1000, delta_weight_grams/1000/reference_calculated_masses[0])
    masses_with_error_1 = reference_calculated_masses[0] + delta_weight_grams / 1000, reference_calculated_masses[1]
    lwc1, mvd1, rss1 = mc.find_lwc_mvd_from_dist(tk, u, p, masses_with_error_1, time, assumed_distribution, ice_density)
    ...
    rows.append([f"{case}", f"{delta_text}", f"{lwc_percent:.1f}", f"{mvd_percent:.1f}"])
    ...
    lwc_percents = []
    mvd_percents = []
    masses_with_error_2 = reference_calculated_masses[0], reference_calculated_masses[1] + delta_weight_grams / 1000
    lwc2, mvd2, rss2 = mc.find_lwc_mvd_from_dist(tk, u, p, masses_with_error_2, time, assumed_distribution, ice_density)
    ...
    lwc2, mvd2, rss2 = mc.find_lwc_mvd_from_dist(tk, u, p, reference_calculated_masses, time, assumed_distribution,
    ...
    lwc1, mvd1, rss1 = mc.find_lwc_mvd_from_dist(tk, u, p, reference_calculated_masses, time + delta_value, assumed_distribution,
                                                 ice_density,)
    lwc_percents.append(calc_percent_difference(lwc, lwc1))
    mvd_percents.append(calc_percent_difference(mvd, mvd1))
    lwc2, mvd2, rss2 = mc.find_lwc_mvd_from_dist(tk, u, p, reference_calculated_masses, time - delta_value, assumed_distribution,
                                                 ice_density)
    lwc_percents.append(calc_percent_difference(lwc, lwc2))
    mvd_percents.append(calc_percent_difference(mvd, mvd2))
    lwc_percent = max([abs(_) for _ in lwc_percents])
    ...
    lwc_percents = []
    mvd_percents = []
    lwc1, mvd1, rss1 = mc.find_lwc_mvd_from_dist(tk, u + 0.44704 * delta_value, p, reference_calculated_masses, time, assumed_distribution, ice_density)
    lwc_percents.append(calc_percent_difference(lwc, lwc1))
    mvd_percents.append(calc_percent_difference(mvd, mvd1))
    lwc1, mvd1, rss1 = mc.find_lwc_mvd_from_dist(tk, u - 0.44704 * delta_value, p, reference_calculated_masses, time, assumed_distribution, ice_density)
    lwc_percents.append(calc_percent_difference(lwc, lwc1))
    ...
    for dist in ('langmuir a', 'langmuir b', 'langmuir c', 'langmuir d', 'langmuir e'):
        dist_calculated_masses, ems, dias = mc.calc_masses_average_ems_diameters(tk, p, u, lwc, mvd, time, dist, ice_density)
        lwc1, mvd1, rss1 = mc.find_lwc_mvd_from_dist(tk, u, p, dist_calculated_masses, time, assumed_distribution, ice_density)
        lwc_percents.append(calc_percent_difference(lwc, lwc1))
        mvd_percents.append(calc_percent_difference(mvd, mvd1))
    ...
    rows =[
        ["Maximum total error (sum(E))", f"{lwc_total_error:.1f}", f"{mvd_total_error:.1F}"],
    ...
    plt.figure()
    for dist in ('langmuir a', 'langmuir b', 'langmuir c', 'langmuir d', 'langmuir e'):
        lwc1, mvd1, rss1 = mc.find_lwc_mvd_from_dist(tk, u, p, masses_with_error_1, time, dist, ice_density)
        print('         ', dist, rss1)
        plt.plot(dist, rss1, '+')
    plt.yscale('log')
    ...
    vs = []
    for dist in ('langmuir a', 'langmuir b', 'langmuir c', 'langmuir d', 'langmuir e'):
        lwc1, mvd1, rss1 = mc.find_lwc_mvd_from_dist(tk, u, p, reference_calculated_masses, time, dist, ice_density)
        print(dist, lwc1, mvd1, rss1, calc_percent_difference(lwc, lwc1), calc_percent_difference(mvd, mvd1))
        vs.append((mvd1, lwc1, dist))
    plt.figure()
    plt.suptitle('Reference "C" compared to dist')
    ...
    print(mvd, lwc)
    plt.figure()
    plt.suptitle('Reference dist compared to "C"')
    for mvd1, lwc1, dist in vs:
        plt.plot(mvd1, lwc1, 'o', label=dist)
    plt.legend()
    ...
    p = multicylinder.langmuir_cylinder.calc_pressure(altitude_ft*12*0.025)
    u = airspeed_mph*.44704
    d_cylinder = 3 * 0.0254
    ...
    for d_drop in d_drops:
        em = multicylinder.langmuir_cylinder.calc_em(tk, p, u, d_drop, d_cylinder)
