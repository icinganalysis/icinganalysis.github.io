...
def calc_mass_average_em_diameter_with_distribution(
    ...
    for i in range(10):
        ...
        mass_initial = (
            em_initial * lwc / G_PER_KG * u * initial_diameter * length * time_in_icing
        )
...
if __name__ == "__main__":
    ...
    p = air_density / (0.3484 / 100 / tk)  # eq. 15, re-arranged
    ...
    print()
    print(
        f"Calculation with k*phi(mvd)=constant\nMVD={mvd:.1f} {dist} Kφ(mvd)={k_phi:.0f}"
    )
    ...
    #  Use the "calc_em_with_distribution_k_phi_mvd" method
    plt.figure()
    from icinganalysis import langmuir_blodgett_multicylinder_k_phi_unique_mvd

    mc2 = langmuir_blodgett_multicylinder_k_phi_unique_mvd.Multicylinder(ds, lengths)
    lwc, mvd, dist, rss = mc2.find_lwc_mvd_dist(
        tk, u, p, mass_rates, time_in_icing, ice_density
    )
    k_phi = calc_k_phi(tk, p, u, mvd)
    d_cyl = [
