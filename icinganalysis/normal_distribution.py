...
if __name__ == "__main__":
    ...
    cumulative_volumes = (
        ...
        0.95,
        1,
    )  # assumed 0 starting point
    assumed_bin_mid_point_cumulative_volumes = (0.025, 0.1, 0.25, 0.5, 0.75, 0.9, 0.975)
    ds_for_assumed_bin_mid_point_cumulative_volumes = q.ppf(
        assumed_bin_mid_point_cumulative_volumes
    )
    size_ratios_for_assumed_bin_mid_point_cumulative_volumes = [
        _ / mvr for _ in ds_for_assumed_bin_mid_point_cumulative_volumes
    ]
    print(
        "size ratios for assumed_bin_mid_point_cumulative_volumes =",
        ", ".join(
            [
                f"{_:.2f}"
                for _ in size_ratios_for_assumed_bin_mid_point_cumulative_volumes
            ]
        ),
    )
    normal_cumulative_volumes = (
        0.01,
        0.05,
        0.15,
        0.35,
    ...
    size_ratios_for_normal_cumulative_volumes = [
        _ / mvr for _ in ds_for_normal_cumulative_volumes
    ]
    print(
        "size ratios for normal_cumulative_volumes =",
        ", ".join([f"{_:.2f}" for _ in size_ratios_for_normal_cumulative_volumes]),
    )
    arithmetic_mids_for_normal_cumulative_volumes = [
        (v1 + v2) / 2 / mvr
        for v1, v2 in zip(
            ds_for_normal_cumulative_volumes[:-1], ds_for_normal_cumulative_volumes[1:]
        )
    ]
    print(
        "mids of size ratios for normal_cumulative_volumes =",
        ", ".join([f"{_:.2f}" for _ in arithmetic_mids_for_normal_cumulative_volumes]),
    )
    ...
    plt.plot(
        ps,
        zs,
        label=f"Houghton and Radford data best fit, MVD={mvr:.2f}, stdev={dev:.2f}",
    )
    ...
    pxs = (0.05, 0.15, 0.35, 0.65, 0.85, 0.95)
    dxs = q.ppf(pxs)
    ...
    cpms = (0.025, 0.1, 0.25, 0.5, 0.75, 0.9, 0.975)
    dxs = q.ppf(cpms)
    # plt.plot(cpms, dxs, 'x')
    ...
    plt.plot(
        zs,
        ps,
        label=f"Houghton and Radford data best fit\nMVR={mvr:.2f}, stdev={dev:.2f}",
    )
    ...
    pxs = (0.01, 0.05, 0.15, 0.35, 0.65, 0.85, 0.95, 0.99)
    dxs = q.ppf(pxs)
    ...
    for dl, cm in zip(dls, assumed_bin_mid_point_cumulative_volumes):
        plt.text(
            dl, cm, f"({dl / mvr:.3f}, {cm:.3f})", color="g", verticalalignment="top"
        )
