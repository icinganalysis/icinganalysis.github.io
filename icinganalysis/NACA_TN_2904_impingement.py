...
from icinganalysis.langmuir_cylinder import calc_k, calc_phi
from icinganalysis.langmuir_cylinder import langmuir_lwc_fractions
from icinganalysis.langmuir_cylinder import langmuir_a_mids, langmuir_b_mids, langmuir_c_mids
from icinganalysis.langmuir_cylinder import langmuir_d_mids, langmuir_e_mids
...
"""
Langmuir A ems 50000  0.005, 0.038, 0.105, 0.254, 0.534, 0.714, 0.825, 0.901, 0.939, 1.000
Langmuir B ems 50000  0.010, 0.043, 0.112, 0.255, 0.520, 0.699, 0.815, 0.898, 0.936, 0.998
Langmuir C ems 50000  0.015, 0.050, 0.120, 0.260, 0.511, 0.684, 0.803, 0.893, 0.934, 0.996
Langmuir D ems 50000  0.022, 0.062, 0.135, 0.271, 0.506, 0.670, 0.789, 0.886, 0.930, 0.994
Langmuir E ems 50000  0.031, 0.075, 0.149, 0.281, 0.499, 0.654, 0.773, 0.875, 0.924, 0.992
...
data_fig_6 = {  # phi
    ...
    10000: {
        'ks': (.125, .25, .5, 1, 4, 16, 40, 320, 500, 1000),
        'ems': (0, .01, .07, .157, .48, .755, .845, .955, .97, .99),
    },
    50000: {
        'ks': (.125, .25, .5, 1, 4, 16, 40, 320, 500, 1000),
        'ems': (0, .005, .038, .105, .378, .682, .795, .94, .96, .987),
    }
}
...
data_fig_6a = {  # phi
    ...
    100: {
        'ks': (.125, .25, .5, 1, 4, 16, 40, 320),
        'ems': (0, .035, .155, .31, .68, .87, .92, .98),
    },
    1000: {
        'ks': (.125, .25, .5, 1, 4, 16, 40, 320),
        'ems': (0, .025, .11, .25, .61, .83, .89, .97),
    },
    10000: {
        'ks': (.125, .25, .5, 1, 4, 16, 40, 320),
        'ems': (0, .01, .06, .165, .48, .755, .845, .955),
    },
    50000: {
        'ks': (.125, .25, .5, 1, 4, 16, 40, 320),
        'ems': (0, .005, .04, .105, .38, .68, .795, .94),
    }
}
# fmt: on
...
def calc_em_naca_tn_2904_from(k, phi, include_compressibility=False):
    partial_em = float(max(0, _em_interpolator_log10(log10(k),
...
def calc_em_naca_tn_2904_from_with_distribution(k, phi, distribution="Langmuir A", include_compressibility=False):
    mids = {
    ...
    for d_drop_ratio, w in zip(mids, langmuir_lwc_fractions):
        em_partial = calc_em_naca_tn_2904_from(k * d_drop_ratio ** 2, phi, include_compressibility)
        em += w * em_partial
    return min(1, max(em, 0))


def calc_em_naca_tn_2904(tk, p, u, mvd, d_cylinder, include_compressibility=False):
...
def calc_em_naca_tn_2904_with_distribution(tk, p, u, mvd, d_cylinder, distribution="Langmuir A",
    ...
    em = 0
    for d_drop_ratio, w in zip(mids, langmuir_lwc_fractions):
        em += w * calc_em_naca_tn_2904(tk, p, u, mvd * d_drop_ratio, d_cylinder, include_compressibility)
    return em
...
_k_interps = {
    'Langmuir A': (
        interp1d(log10_ks, data_table_iv_augmented[0]['ems_a'], kind, fill_value='extrapolate'),
        interp1d(log10_ks, data_table_iv_augmented[200]['ems_a'], kind, fill_value='extrapolate'),
        interp1d(log10_ks, data_table_iv_augmented[1000]['ems_a'], kind, fill_value='extrapolate'),
        interp1d(log10_ks, data_table_iv_augmented[3000]['ems_a'], kind, fill_value='extrapolate'),
        interp1d(log10_ks, data_table_iv_augmented[10000]['ems_a'], kind, fill_value='extrapolate'),
        interp1d(log10_ks, data_table_iv_augmented[50000]['ems_a'], kind, fill_value='extrapolate'),
    ),
    'Langmuir B': (
        interp1d(log10_ks, data_table_iv_augmented[0]['ems_b'], kind, fill_value='extrapolate'),
        interp1d(log10_ks, data_table_iv_augmented[200]['ems_b'], kind, fill_value='extrapolate'),
        interp1d(log10_ks, data_table_iv_augmented[1000]['ems_b'], kind, fill_value='extrapolate'),
        interp1d(log10_ks, data_table_iv_augmented[3000]['ems_b'], kind, fill_value='extrapolate'),
        interp1d(log10_ks, data_table_iv_augmented[10000]['ems_b'], kind, fill_value='extrapolate'),
        interp1d(log10_ks, data_table_iv_augmented[50000]['ems_b'], kind, fill_value='extrapolate'),
    ),
    'Langmuir C': (
        interp1d(log10_ks, data_table_iv_augmented[0]['ems_c'], kind, fill_value='extrapolate'),
        interp1d(log10_ks, data_table_iv_augmented[200]['ems_c'], kind, fill_value='extrapolate'),
        interp1d(log10_ks, data_table_iv_augmented[1000]['ems_c'], kind, fill_value='extrapolate'),
        interp1d(log10_ks, data_table_iv_augmented[3000]['ems_c'], kind, fill_value='extrapolate'),
        interp1d(log10_ks, data_table_iv_augmented[10000]['ems_c'], kind, fill_value='extrapolate'),
        interp1d(log10_ks, data_table_iv_augmented[50000]['ems_c'], kind, fill_value='extrapolate'),
    ),
    'Langmuir D': (
        interp1d(log10_ks, data_table_iv_augmented[0]['ems_d'], kind, fill_value='extrapolate'),
        interp1d(log10_ks, data_table_iv_augmented[200]['ems_d'], kind, fill_value='extrapolate'),
        interp1d(log10_ks, data_table_iv_augmented[1000]['ems_d'], kind, fill_value='extrapolate'),
        interp1d(log10_ks, data_table_iv_augmented[3000]['ems_d'], kind, fill_value='extrapolate'),
        interp1d(log10_ks, data_table_iv_augmented[10000]['ems_d'], kind, fill_value='extrapolate'),
        interp1d(log10_ks, data_table_iv_augmented[50000]['ems_d'], kind, fill_value='extrapolate'),
    ),
    'Langmuir E': (
        interp1d(log10_ks, data_table_iv_augmented[0]['ems_e'], kind, fill_value='extrapolate'),
        interp1d(log10_ks, data_table_iv_augmented[200]['ems_e'], kind, fill_value='extrapolate'),
        interp1d(log10_ks, data_table_iv_augmented[1000]['ems_e'], kind, fill_value='extrapolate'),
        interp1d(log10_ks, data_table_iv_augmented[3000]['ems_e'], kind, fill_value='extrapolate'),
        interp1d(log10_ks, data_table_iv_augmented[10000]['ems_e'], kind, fill_value='extrapolate'),
        interp1d(log10_ks, data_table_iv_augmented[50000]['ems_e'], kind, fill_value='extrapolate'),
    ),
}
...
def interp_table_iii_em_from_k_phi(k, k_phi, distribution='Langmuir A'):
    return float(max(0, _k_phi_interpolators[distribution](log10(1 / k),
                                                           log10(k_phi) if phi > 0 else l10phi_for_k_phi_0
                                                           )))
...
def make_table_iii():
    ...
    for i, inv_k in enumerate(data_table_iv_augmented[k_phi]['inv_ks']):
        ...
        for dr, w in zip(langs[distribution], langmuir_lwc_fractions):
            ...
            # emz +=
        print(k_phi, phi, inv_k, distribution, emx, ems, emy, data_table_iv_augmented[k_phi]['ems_c'][i])
    k_phi = 50000
    for distribution in (
        "Langmuir A",
        "Langmuir B",
...
def make_table_i():
    ...
    for i, k in enumerate((.25, .5, 1, 4, 16, 40, 320)):
        ...
        row = f"{phi:.0f}", f"{k:.2f}", f"{em:.3f}", f"{float(em_langmuir_blodgett):.3f}", f"{emx:.3f}"
        rows.append(row)
    print()
    phi = 100
    ...
    for k_phi in (0, 200, 1000, 3000, 10000):
        ...
        for i, inv_k in enumerate(data_table_iv_augmented[k_phi]['inv_ks']):
            ...
            for dist, tab in langs_table_iv.items():
                ...
                emc = calc_em_naca_tn_2904_from_with_distribution(1 / inv_k, k_phi * inv_k, dist, False)
                diff = emc - ema
...
if __name__ == '__main__':
    ...
    for k_phi in data_table_iv_augmented:
        ...
        line, = plt.plot(ks, data_table_iv_augmented[k_phi]["ems_a"], 'o',
                         fillstyle='none', label=f"Table IV k_phi={k_phi}")
        emx = [calc_em_naca_tn_2904_from(k, k_phi / k) for k in ks]
    ...
    for k_phi in data_table_iv_augmented:
        line, = plt.plot(data_table_iv_augmented[k_phi]["inv_ks"], data_table_iv_augmented[k_phi]["ems_a"], 'o',
