...
d_inch = 0.0007, 0.001, 0.002, 0.004, 0.006, 0.01, 0.04
d_micrometer = [_ * 0.0254 * 1000000 for _ in d_inch]
em_points = 0.075, 0.2, 0.52, 0.78, 0.86, 0.92, 0.99  # fig. 4
ems_table_iv = 0.25, 0.445, 0.76, 0.928, 0.966, 0.987, 0.999

ks_points = []
for dm in d_micrometer:
    k = langmuir_cylinder_values.calc_k(tk, u, dm, d_cylinder)
    ks_points.append(k)

plt.figure(figsize=(7, 6))
(line0,) = plt.plot(
    ks_points, em_points, "o", c="g", label="NACA-TN-779 Figure 4 points"
)

d_drops_micrometer = [
    10 ** _
    for _ in plt.np.arange(log10(8), log10(4000), (log10(4000) - log10(8)) / 100)
]

phi_target = 0.1  # use as an approximation for phi=0
p = minimize_scalar(
    lambda _: abs(langmuir_cylinder_values.calc_phi(tk, _, u, d_cylinder) - phi_target)
).x
...
ems = [
    langmuir_cylinder_values.calc_em(tk, p, u, d_drop_m, d_cylinder)
    for d_drop_m in d_micrometer
]
ks = [
    langmuir_cylinder_values.calc_k(tk, u, d_drop_m, d_cylinder)
    for d_drop_m in d_micrometer
]
(line,) = plt.plot(ks, ems, label=f"Calculated φ={phi:.0f}")
plt.plot(
    ks_points, ems_table_iv, "s", c=line.get_color(), label="Table IV(a) D.A. points"
)
...
(line,) = plt.plot(
    ...
    ems,
    c=line0.get_color(),
    label=f"Calculated φ={phi:.0f}, altitude={altitude:.0f} m",
)
ems = [
    langmuir_cylinder_values.calc_em(tk, p, u, d_drop_m, d_cylinder)
    for d_drop_m in d_micrometer
...
plt.plot(
    ...
    c=line.get_color(),
    ms=12,
    label=f"Revised points at φ={phi:.0f}",
)
...
(line,) = plt.plot(
    ks, ems, c="r", label=f"Calculated φ={phi:.0f}, altitude={altitude:.0f} m"
)
plt.plot(
    ks_points,
    ems,
