...
def calc_mass_average_em_diameter_with_distribution(
    ...
    for i in range(10):
        em_initial = langmuir_cylinder.calc_em_with_distribution(
            tk, p, u, mvd, initial_diameter, distribution
        )
        mass_initial = (
            em_initial * lwc / 1000 * u * initial_diameter * length * time_in_icing
        )
        em_final = langmuir_cylinder.calc_em_with_distribution(
...
class Multicylinder:
    ...
    def calc_ems_fixed_diameters(self, tk, ms, p, mvd, distribution="Langmuir A"):
        ...
        for d, length in zip(self.diameters, self.lengths):
            em = langmuir_cylinder.calc_em_with_distribution(
                tk, p, ms, mvd, d, distribution
            )
            ems.append(em)
        return ems
...
if __name__ == "__main__":
    ...
    plt.figure()
    plt.suptitle(f"Calculation with k*phi unique for each drop size bin\nMVD={mvd:.1f} a={mvd/2:.2f} {dist} Kφ(mvd)={k_phi:.0f}")
